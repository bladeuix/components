...
it(description: 'can render tabs with style, placement, and size classes', closure: function () {
    $view = $this->blade(template: '<x-daisyui::tabs style="lift" placement="bottom" size="lg">Tabs</x-daisyui::tabs>');
...
it(description: 'ignores unsupported tabs modifiers', closure: function () {
    $view = $this->blade(template: '<x-daisyui::tabs style="invalid" placement="invalid" size="invalid" />');
...
it(description: 'can render a radio tab label with an optional icon and content', closure: function () {
    $view = $this->blade(
        template: '<x-daisyui::tabs><x-daisyui::tab name="profile" label="Profile" active :icon="$icon" class="border-base-300">Profile content</x-daisyui::tab></x-daisyui::tabs>',
...
it(description: 'can render an active disabled radio tab', closure: function () {
    $view = $this->blade(template: '<x-daisyui::tab name="profile" label="Profile" active disabled />');
...
it(description: 'does not allow radio tab attributes to override its fixed attributes', closure: function () {
    $view = $this->blade(template: '<x-daisyui::tab name="profile" label="Profile" type="checkbox" autocomplete="on" aria-label="Overridden">Profile</x-daisyui::tab>');
