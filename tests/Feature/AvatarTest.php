...
it(description: 'can render an avatar image', closure: function () {
    $view = $this->blade(template: '<x-daisyui::avatar image-src="https://example.com/avatar.webp" image-alt="Jane Doe" />');
...
it(description: 'can render a rounded avatar', closure: function () {
    $view = $this->blade(template: '<x-daisyui::avatar rounded image-src="https://example.com/avatar.webp" image-alt="Jane Doe" />');
...
it(description: 'can render online and offline avatar statuses', closure: function (string $status) {
    $view = $this->blade(template: sprintf('<x-daisyui::avatar presence="%s" image-src="https://example.com/avatar.webp" image-alt="Avatar" />', $status));
...
it(description: 'does not clip presence indicators on rounded avatars', closure: function () {
    $view = $this->blade(template: '<x-daisyui::avatar rounded presence="online" image-src="https://example.com/avatar.webp" image-alt="Avatar" />');
...
it(description: 'uses an abbreviation instead of an image', closure: function () {
    $view = $this->blade(template: '<x-daisyui::avatar abbreviation="AI" image-src="https://example.com/avatar.webp" image-alt="Avatar" />');
...
it(description: 'merges component attributes onto the avatar', closure: function () {
    $view = $this->blade(template: '<x-daisyui::avatar image-src="https://example.com/avatar.webp" image-alt="Jane Doe" loading="lazy" class="w-24 rounded-full" />');
